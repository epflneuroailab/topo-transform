...
class RidgeGCVTorch:
    ...
    def fit(
        ...
        for i, alpha in enumerate(alphas_vec):
    ...
    def predict(self, X: TensorLike, ret_numpy=False) -> np.ndarray:
        ...
        if self.coef_.ndim == 1:
            ...
            yhat = yhat + bias
        return _to_numpy(yhat) if ret_numpy else yhat
...
if __name__ == "__main__":
    ...
    n_samples = 100
    n_features_list = [2, 10, 100, 1000]  # test both n>p and n<p
    ...
    for n_features in n_features_list:
        ...
        yhat = model.predict(X_test, ret_numpy=True)
        ...
        print("Max prediction diff:", np.abs(yhat - yhat_sk).max())